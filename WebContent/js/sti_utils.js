 ...
 * Get the direct child element of a given element with the given name
    搜索parent_el元素对象中的子元素名为ch_name的第一个元素
 */
...
/*
 * Get the direct child element of a given element with the given name
    搜索parent_el元素对象中的子元素名为ch_name的子元素集合
 */
function get_childs_element(parent_el, ch_name)
{
    var nl = parent_el.childNodes;
    ...
    for(var i=0;i<nl.length;i++){
        var n=nl[i];
        if (n.nodeType == Node.ELEMENT_NODE) {
            if(typeof(n)!='undefined'&&n.tagName == ch_name){
...
//attrValue:属性值
//该方法根据这三个参数可得到该对象
function get_element_obj(eroot,attrName,attrValue)
{

    for (var i=0; i<eroot.childNodes.length; i++) {
        var ch = eroot.childNodes[i];
        ...
        var objValue = ch.getAttribute(attrName);
